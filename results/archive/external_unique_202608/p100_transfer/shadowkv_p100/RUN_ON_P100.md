...
MeritKV's own admit/bypass labels. Phase 1 runs `no_cache`, `shadow_kv`, and
`shadow_kv_plus`; Phase 2 trains from the `shadow_kv_plus` trace rows. Phase 1 uses deterministic epsilon
...
The exploration draw includes the request index, so repeated templated requests
still receive independent train-time exploration decisions.
...
Training pools the full learned-baseline train corpus: seeds {42, 123}, 5
datasets, and both templated and semantic modes. It then evaluates on seed 456
and the remaining 5 datasets. This split keeps the Phase 3 comparison
out-of-sample. The learned policies also tune a small C grid (`0.1, 1, 10`) and
admit-threshold grid on a deterministic validation split; the selected
threshold is saved in the policy JSON and used automatically by the learned
engine.

The learned label is a realized single-request net-savings label, so the
package now also runs a separate memory-bound victim/recovery trace. That trace
tests whether learned admission preserves useful prefixes under eviction
pressure instead of only optimizing the immediate request.

```bash
python experiments/run_learned_baseline.py     # -> results/learned_baseline/
```

For the complete learned-baseline matrix used for reporting, run the wrapper
once:
...
This runs GPT-2 first and Phi-3 last over five seeds
`42, 123, 456, 789, 999`. For each model it trains on templated + semantic
traffic, evaluates on raw + templated + semantic traffic, and then runs the
memory-bound victim/distractor/recovery trace. Outputs are written under:
...
The wrapper uses `--resume` by default, so rerunning the same command skips
completed benchmark cells and continues from the first missing cell.

The memory-bound trace warms each victim prefix twice, runs repeated
distractor pressure, then probes victim recovery. Its per-model outputs are
under:

```text
results/learned_baseline_5seed_all_models/<model_slug>/memory_bound/
...
hit-rate for `no_cache`, `shadow_kv`, `shadow_kv_plus_lite`,
`shadow_kv_plus`, and `shadow_kv_plus_learned`. In this package,
`shadow_kv_plus_lite` is the capacity/break-even-style comparator to keep the
learned result from being read only against MeritKV. The learned engine also reports
...
For the locality mechanism claim, use
`summary_memory_bound_all_models.json`. The important fields are
`victim_recovery_hit_rate`, `first_recovery_hit_rate`,
`victim_recovery_speedup_vs_no_cache`, `evictions`, `peak_memory_used_mb`, and
the learned-policy flip totals.
