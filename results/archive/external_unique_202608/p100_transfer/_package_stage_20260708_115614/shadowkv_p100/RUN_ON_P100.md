...
Compares MeritKV's hand-derived utility formula against a real learned
admission engine. The driver trains a logistic regression from ShadowKV++
policy traces using decision-time features only, saves it as
`results/learned_baseline/learned_policy.json`, then runs
`shadow_kv_plus_learned` on held-out datasets/seeds beside `no_cache`,
`shadow_kv`, and `shadow_kv_plus`.
...
```
This uses `results/learned_baseline/learned_policy.json`. If that JSON is
missing but the Phase 1 train `policy_trace.jsonl` files are still present, the
driver will rebuild the policy before starting Phase 3.
...
**What to look at:** `phase3_summary.json` reports held-out latency, speedup,
waste, and hit-rate for `no_cache`, `shadow_kv`, `shadow_kv_plus`, and
