...
NATURAL_SHARED_TEXTS = (
    "A production language-model service receives a request together with its prompt, arrival metadata, and cache state. The controller examines an exact-prefix candidate before deciding whether reuse is economically worthwhile. The decision must account for saved prefill computation, lookup overhead, memory pressure, and the expected cost of retaining an entry that may never be used again. ",
    "Consider an evaluation in which two requests begin with the same instructions and contextual document but ask different questions afterward. A correct exact-prefix cache stores only the token-identical portion. Reusing that prefix should preserve the causal state while the distinct suffix is processed normally with its own positions and attention mask. ",
    "The experiment records three paths for every example: a complete recomputation, ordinary continuation from a prefix-only cache, and continuation from a longer request whose cache has been cropped at the shared boundary. Comparing these paths distinguishes ordinary floating-point variation from an error introduced specifically by cache splicing. ",
    "Reliable systems evidence requires the model revision, software versions, numerical precision, attention implementation, token positions, and mask construction to be stated explicitly. Token-level comparisons reveal changes in greedy decoding, while logit-level distances show whether a disagreement is associated with a broad distribution shift or a near-tied argmax. ",
    "During multi-round serving, cache admission and eviction affect later requests because unnecessary entries can displace prefixes that will soon be reused. A utility-aware controller therefore observes realized hits and waste, updates its estimates, and bypasses storage when the predicted benefit does not cover the relevant computational and capacity costs. ",
    "This validation focuses only on token-identical exact prefixes. Approximate semantic reuse is a different mechanism because similar text does not guarantee interchangeable hidden states. Any semantic substitution needs a separate correctness guard and should not inherit the correctness interpretation of exact-prefix caching. ",
    "A careful report separates compatibility measurements from enforced performance measurements. A write-through overlay can establish that decision computation adds little overhead, but acceleration attributable to admission requires a path in which the decision actually changes cache lookup, insertion, or eviction behavior. ",
    "The purpose of this control is not to demand bitwise identity from half-precision GPU kernels. Instead, it measures the splice against both full recomputation and the supported native-cache continuation, retaining finite-logit checks, complete greedy-token traces, and distributional differences for every tested case. ",
)

NATURAL_SOURCE_SUFFIXES = (
    "Summarize the operational tradeoff and identify the condition under which retaining the prefix is beneficial.",
    "Explain how the cache entry influences the next request and list the measurements needed for verification.",
    "Describe the baseline execution path and give a concise interpretation of the observed latency distribution.",
    "State the assumptions behind the experiment and explain why the comparison is reproducible.",
    "Analyze the capacity-pressure scenario and report which cached item should remain resident.",
    "Explain why approximate matching requires an independent output-quality safeguard.",
    "Compare the write-through integration with an enforced admission decision in practical serving.",
    "Interpret the logit comparison and determine whether the discrepancy exceeds ordinary cache variation.",
)

NATURAL_MODIFIED_SUFFIXES = (
    "Give two reasons the same prefix should be bypassed when memory pressure and expected waste are both high.",
    "Describe how a different follow-up request should continue from the identical cached context without reusing its suffix.",
    "Identify what the experiment cannot prove when the controller merely logs its decision but does not enforce it.",
    "Explain which token positions and attention-mask entries are required after the shared prefix boundary.",
    "Discuss how recency-aware and frequency-aware baselines help isolate the contribution of utility-based admission.",
    "State why token-identical exact reuse and semantically similar reuse require different safety claims.",
    "Explain what evidence would demonstrate end-to-end acceleration caused specifically by the admission controller.",
    "Compare an isolated argmax flip with widespread output corruption and say what additional control resolves it.",
)
...
def make_natural_pair(
    ...
    """Construct two natural-text token sequences with an exact shared prefix."""
    index = sample % len(NATURAL_SHARED_TEXTS)
    shared_text = " ".join(
        NATURAL_SHARED_TEXTS[(index + offset) % len(NATURAL_SHARED_TEXTS)]
        for offset in range(len(NATURAL_SHARED_TEXTS))
    )
    shared_ids = tokenizer.encode(shared_text, add_special_tokens=False)
    if len(shared_ids) < shared_length:
        raise RuntimeError("Natural shared-text corpus tokenized shorter than requested prefix")

    suffix_length = sequence_length - shared_length
    source_text = " ".join(
        NATURAL_SOURCE_SUFFIXES[(index + offset) % len(NATURAL_SOURCE_SUFFIXES)]
        for offset in range(len(NATURAL_SOURCE_SUFFIXES))
    )
    modified_text = " ".join(
        NATURAL_MODIFIED_SUFFIXES[(index + offset) % len(NATURAL_MODIFIED_SUFFIXES)]
        for offset in range(len(NATURAL_MODIFIED_SUFFIXES))
    )
    source_suffix = tokenizer.encode(source_text, add_special_tokens=False)
    modified_suffix = tokenizer.encode(modified_text, add_special_tokens=False)
    if len(source_suffix) < suffix_length or len(modified_suffix) < suffix_length:
        raise RuntimeError("Natural suffix corpus tokenized shorter than requested suffix")

    prefix = shared_ids[:shared_length]
    source_ids = prefix + source_suffix[:suffix_length]
    modified_ids = prefix + modified_suffix[:suffix_length]
    source = torch.tensor([source_ids], dtype=torch.long, device=device)
    modified = torch.tensor([modified_ids], dtype=torch.long, device=device)
    if not torch.equal(source[:, :shared_length], modified[:, :shared_length]):
        raise AssertionError("Natural-text prefixes are not token-identical")
    if torch.equal(source[:, shared_length:], modified[:, shared_length:]):
        raise AssertionError("Natural-text suffixes are unexpectedly identical")
...
def main() -> int:
    ...
    parser.add_argument("--input-mode", choices=("random", "natural"), default="random")
    ...
    if args.input_mode == "natural":
        if not args.model_id:
            parser.error("--input-mode natural requires --model-id")
    ...
    for sample in range(args.samples):
        for ratio in args.shared_ratios:
            ...
            if args.input_mode == "natural":
                source_ids, modified_ids = make_natural_pair(
                    tokenizer,
