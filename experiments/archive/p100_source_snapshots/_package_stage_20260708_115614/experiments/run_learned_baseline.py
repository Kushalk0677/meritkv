...
def row_to_features(row: dict) -> dict[str, float] | None:
    ...
    feat = {
        ...
        **flags,
        "label": 1.0 if strategy in {"exact", "semantic_partial"} else 0.0,
    }
    return feat
        ...
        with path.open("r", encoding="utf-8") as fh:
            ...
            for line in fh:
                ...
                if feat is not None:
                    rows.append(feat)
                    count += 1
        print(f"  {path}: {count} usable ShadowKV++ decisions")
    return rows


def matrix(features: list[dict[str, float]]) -> tuple[np.ndarray, np.ndarray]:
...
def train_policy(features: list[dict[str, float]], output_path: Path) -> dict:
    if len(features) < 10:
        raise RuntimeError(f"Need at least 10 trace rows to train; found {len(features)}")
    x, y = matrix(features)
    ...
    if len(labels) < 2:
        ...
        payload = {
            ...
            "label_mode": "imitate_meritkv_policy",
            ...
            "note": "Only one class appeared in the training traces.",
        }
    else:
        try:
        ...
        payload = {
            ...
            "train_bypass": int(len(y) - y.sum()),
            "note": (
                "Trained from logged MeritKV decisions using decision-time features only. "
                "This is an online learned-policy baseline, not an oracle counterfactual labeler."
            ),
        }
    ...
    print(f"  Train samples={payload['train_samples']} admit={payload['train_admit']} bypass={payload['train_bypass']} acc={payload['train_accuracy']:.3f}")
    return payload
...
def main() -> None:
    ...
    policy_path = Path(args.policy_path) if args.policy_path else out_root / "learned_policy.json"
    ...
    if args.phase in {"all", "train"} and not args.skip_policy_training:
        ...
        if args.dry_run:
            print("Dry run: not writing learned policy.")
        else:
            train_policy(features, policy_path)
    elif args.skip_policy_training or args.phase == "phase3":
        if policy_path.exists():
            print(f"\n[Phase 2] Using existing learned policy: {policy_path}")
        elif args.phase == "phase3" and not args.skip_policy_training:
            trace_paths = sorted(Path(p) for p in glob.glob(str(train_dir / "**" / "policy_trace.jsonl"), recursive=True))
            if trace_paths:
                print(f"\n[Phase 2] Rebuilding missing learned policy from {len(trace_paths)} train trace file(s)")
                features = load_trace_features(trace_paths)
                if args.dry_run:
                    print("Dry run: not writing learned policy.")
                else:
                ...
                print(f"\n[Phase 2] Dry run: would use existing learned policy at {policy_path}")
            else:
                raise RuntimeError(
                    f"Expected an existing learned policy at {policy_path}, but none was found. "
            ...
            raise RuntimeError(
                f"Expected an existing learned policy at {policy_path}. "
                "Run --phase train first, or pass --policy_path to the JSON produced by Phase 2."
            )
    ...
    if args.phase in {"all", "phase3"} and not args.skip_test_benchmarks:
        ...
        for seed in TEST_SEEDS:
            for dataset in TEST_DATASETS:
                for mode in MODES:
                    cell = test_dir / model_slug(args.model) / mode / f"seed_{seed}" / dataset
                    ok = run_benchmark_cell(
                        backend=args.backend,
                        model=args.model,
                        device=args.device,
                    ...
                    if not ok:
                        raise RuntimeError(f"Test cell failed: {dataset}/{mode}/seed_{seed}")

    if args.phase in {"all", "phase3", "evaluate"} and not args.dry_run:
        summarize_results(test_dir, out_root / "phase3_summary.json")
