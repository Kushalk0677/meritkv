...
class TieredStateBank:
        ...
        with self._lock:
            ...
            while True:
                ...
                if current_used + memory_bytes <= limit:
                    break
                if not self._evict_one_unlocked(target_tier=tier):
                    # Fallback to evicting from the other tier if needed, but primarily target the requested tier
    ...
    def _find_match_unlocked(self, tokens: Tuple[int, ...]) -> Optional[Tuple[Tuple[int, ...], CacheEntry, int]]:
        ...
        # Fall back to trie traversal for general prefix matching
        ...
        if best_len == len(tokens):
            ...
            self._recent_match_cache_keys.append(tokens)
            # Maintain cache size limits
            if len(self._recent_match_cache_keys) > 64:
                oldest_key = self._recent_match_cache_keys.popleft()
                self._recent_match_cache.pop(oldest_key, None)
    ...
    def _remove_unlocked(self, prefix_tokens: Tuple[int, ...], count_speculative_waste: bool = True) -> None:
        ...
        if prefix_tokens in self._recent_match_cache_keys:
            # Use list conversion for O(1) removal - deque.remove is O(n)
            self._recent_match_cache_keys = deque(
                [k for k in self._recent_match_cache_keys if k != prefix_tokens], 
                maxlen=64
            )
        if entry.tier == 'gpu':
