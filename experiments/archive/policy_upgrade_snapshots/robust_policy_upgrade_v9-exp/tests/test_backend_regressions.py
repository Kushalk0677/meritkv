...
def test_hf_cached_prefill_matches_full_prefill_cache_shape_for_tiny_model():
    ...
    cached = backend.prefill(tokens[prefix_len:], past_key_values=prefix.past_key_values)

    assert backend._past_length(full.past_key_values) == backend._past_length(cached.past_key_values)
