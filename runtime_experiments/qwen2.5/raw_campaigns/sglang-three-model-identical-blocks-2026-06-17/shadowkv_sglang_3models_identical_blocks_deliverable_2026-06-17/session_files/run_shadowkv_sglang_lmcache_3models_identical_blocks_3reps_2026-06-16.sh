...
RUN_ID="sglang_lmcache_shadowkv_3models_identical_blocks_3reps_2026-06-16"
if [[ "$MODE" == "smoke" ]]; then
  RUN_ID="sglang_lmcache_shadowkv_3models_identical_blocks_smoke_2026-06-16"
fi
...
CONFIG_HOST="/home/jade_hand/research/shadowkv/session_files/lmcache_mp_3models_no_native_radix_2026-06-16.yaml"
CONFIG_CONTAINER="/workspace/shadowkv/session_files/lmcache_mp_3models_no_native_radix_2026-06-16.yaml"
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  MODES=(templated)
  MODELS=("qwen25_15b|Qwen/Qwen2.5-1.5B-Instruct")
  REPS=(1)
  N_REQUESTS=16
  WARMUP_REQUESTS=2
fi
...
restore_production() {
  set +e
  docker rm -f shadowkv-3models-cell >/dev/null 2>&1 || true
...
run_one() {
  ...
  local common_docker=(
    docker run --rm --name shadowkv-3models-cell
...
full_csv = root / "aggregate_3models_3baselines_3reps_full.csv"
full_json = root / "aggregate_3models_3baselines_3reps_full.json"
if rows:
...
summary_csv = root / "summary_by_model_baseline_3models_3reps.csv"
summary_json = root / "summary_by_model_baseline_3models_3reps.json"
if summary_rows:
...
paired_csv = root / "paired_deltas_vs_native_radix_3models_3reps.csv"
paired_json = root / "paired_deltas_vs_native_radix_3models_3reps.json"
if paired:
...
paired_summary_csv = root / "paired_delta_summary_by_model_3models_3reps.csv"
paired_summary_json = root / "paired_delta_summary_by_model_3models_3reps.json"
if paired_summary:
...
(root / "SUMMARY_3MODELS_3BASELINES_3REPS.md").write_text("\n".join(lines) + "\n")
print(root / "SUMMARY_3MODELS_3BASELINES_3REPS.md")
