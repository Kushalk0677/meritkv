...
- **Last ShadowKV check:** 2026-06-17 21:20 EDT - Qwen14B identical-block SGLang/Radix/ShadowKV++/patched-LMCache matrix completed, 90/90 cells persisted, aggregate/audit artifacts generated, and production `darwin28b-reason-vllm` was restored.
...
| SHKV-007 | done | 1 | Run the identical-block setup on Qwen2.5-14B. | Smoke passed; 90/90 jobs, 30/30 LMCache summaries, aggregate/audit artifacts, production restore, and Kushal deliverable packaged. |
...
## 2026-06-18 Qwen14B Identical-Blocks SGLang Rerun

- Report: [[shadowkv-sglang-small-model-test-2026-06-13]].
- Purpose: run the exact matched-block setup on `Qwen/Qwen2.5-14B-Instruct`.
- Result root: `/home/jade_hand/research/shadowkv/results_sglang_lmcache_shadowkv_qwen14b_identical_blocks_3reps_2026-06-17`.
- Run script: `/home/jade_hand/research/shadowkv/session_files/run_shadowkv_sglang_lmcache_qwen14b_identical_blocks_3reps_2026-06-17.sh`.
- Status: complete; smoke passed, full matrix `90/90`, LMCache summaries `30/30`, aggregates generated, and audit artifact `ANOMALY_AUDIT_QWEN14B_IDENTICAL_BLOCKS_2026-06-18.md` generated.
- Schedule audit: 30 matched workload blocks; each baseline appears in positions 1/2/3 exactly 10 times; error scan hits `0`.
- Headline: ShadowKV++ improved over native Radix on Qwen14B by `1.73%` mean latency, `2.07%` P95 latency, and `1.76%` throughput. Cached-token totals match native Radix exactly. Patched LMCache no-native-Radix remains functional but slower than native Radix.
- Production restore: `darwin28b-reason-vllm` restarted and health-checked on `127.0.0.1:8015`.
