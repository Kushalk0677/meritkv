...
## [2026-06-18] benchmark | ShadowKV Qwen14B identical-block rerun
- **Author:** Keystone (Codex on IronGod)
- **Scope:** Qwen2.5-14B SGLang/Radix/ShadowKV++/patched-LMCache matrix with the same matched-block setup used for the cleaner three-model rerun.
- **Action:** Ran 30 matched rep/dataset/mode blocks, each containing native Radix, ShadowKV++ overlay, and patched LMCache no-native-Radix at 256 requests per cell.
- **Result:** 90/90 jobs completed, 30/30 LMCache summaries generated, aggregate CSV/JSON/Markdown artifacts generated, and audit artifact `ANOMALY_AUDIT_QWEN14B_IDENTICAL_BLOCKS_2026-06-18.md` recorded zero error scan hits and balanced baseline positions. ShadowKV++ improved over native Radix by `1.73%` mean latency, `2.07%` P95 latency, and `1.76%` throughput; cached-token totals matched native Radix exactly.
- **Artifacts:** Result root `/home/jade_hand/research/shadowkv/results_sglang_lmcache_shadowkv_qwen14b_identical_blocks_3reps_2026-06-17`; report updated at `Wiki/life/projects/shadowkv-sglang-small-model-test-2026-06-13.md`; task queue updated at `Wiki/life/projects/nemotron-shadowkv-task-queue.md`.
- **Verification:** Production `darwin28b-reason-vllm` was restored and `http://127.0.0.1:8015/v1/models` returned `darwin28b-reason`.
