#!/usr/bin/env python3
"""Gemma capacity-pressure client with independent skip-write/lookup actions.
    ...
    try:
        ...
        for metric_name in sorted(required_samples):
            ...
            if definition in definitions:
                key = f'prometheus::{metric_name}{{cache_type="SWARadixCache"}}'
                values[key] = 0.0
...
def capacity_save(output_dir: str, filename: str, payload: dict[str, Any]) -> Path:
    ...
    for key, metrics in payload.items():
        ...
        metrics["capacity_pressure_retention_policy"] = "lru"
        metrics["capacity_pressure_cache_implementation"] = "SWARadixCache"
        metrics["capacity_pressure_hybrid_swa_memory"] = True
    ...
    payload["meritkv_capacity_extension"] = {
        ...
        "cache_implementation": "SWARadixCache",
