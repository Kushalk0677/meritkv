...
LFU_FEASIBILITY_RECEIPT = "feasibility/lfu_infeasibility_receipt.json"
...
def main() -> None:
    ...
    if len(full_rows) != 20 or len(smoke_rows) != 4:
        raise SystemExit(
    ...
    for seed in EVALUATION_SEEDS:
        ...
        if len(seed_arms) != 4 or set(seed_arms) != set(ARMS):
    ...
    if any(
        sorted(counts.values()) != [1, 1, 1, 2]
        for counts in position_counts.values()
    ):
        raise SystemExit(f"evaluation position imbalance exceeded frozen bound: {position_counts}")

    feasibility_receipt_path = HERE / LFU_FEASIBILITY_RECEIPT
    if not feasibility_receipt_path.is_file():
        raise SystemExit(f"missing frozen LFU feasibility receipt: {feasibility_receipt_path}")
    manifest = {
        "schema_version": 1,
        ...
        "arms": ARMS,
        "cache_topology": "default Gemma SWARadixCache with effective LRU retention",
        "evaluation_design": {
            "cells": len(full_rows),
            ...
            "balance_rule": (
                "four cyclic Latin rows followed by the first row; over five seeds every arm "
                "appears once per seed and in every position, with one position repeated"
            ),
        },
        "smoke_design": {
        ...
        "optional_lfu_baseline": {
            "status": "infeasible_on_pinned_image",
            "scientific_disposition": "omitted_as_infeasible",
            "receipt": LFU_FEASIBILITY_RECEIPT,
            "receipt_sha256": hashlib.sha256(feasibility_receipt_path.read_bytes()).hexdigest(),
        },
        "traces": {},
    }
