#!/usr/bin/env python3
"""Guarded First Light controller for the frozen Gemma capacity extension."""
...
PACKET = PROJECT / "meritkv_capacity_pressure_20260822"
RESULTS = Path("/datapool/experiments/meritkv/gemma4_capacity_pressure_20260823")
...
MODEL_SNAPSHOT = Path(
    "/datapool/cache/huggingface/hub/models--google--gemma-4-31B-it/"
    "snapshots/b9ea41a2887d8607f594846523f94c6cc75ac8a4"
)
CONTAINER_SNAPSHOT = (
    "/hf_hub/models--google--gemma-4-31B-it/"
    "snapshots/b9ea41a2887d8607f594846523f94c6cc75ac8a4"
)
TOKENIZER_MANIFEST = "202cc4cc84e68aa6ec005e5b35a0627b4de805cfe1df61d5c5fc5b1f25fcca0e"
MODEL_CONFIG_HASH = "e967dd38bc5cfd38bd09a995a7bf4a754075df2b46aba68f7fbb5a791e6d8dd1"
MODEL_INDEX_HASH = "d4aff3b976d69c123a29d1c085d7ba4de1ac3f4ca1726a7f81e1b11462a64ea2"
...
SERVER_NAME = "meritkv-capacity-gemma-server"
CLIENT_NAME = "meritkv-capacity-cell-client"
SERVER_PORT = 30000
MAX_TOTAL_TOKENS = 16384
PAGE_SIZE = 1
OUTAGE_NOT_BEFORE = datetime.fromisoformat("2026-08-22T22:30:00-04:00")
...
class Controller:
    ...
    def tokenizer_manifest_hash(self) -> str:
        files = (
            "chat_template.jinja",
        ...
        records = sorted(
            f"{sha256(MODEL_SNAPSHOT / name)}  {name}\n" for name in files
        )
    ...
    def preflight(self) -> None:
        ...
        if sha256(MODEL_SNAPSHOT / "config.json") != MODEL_CONFIG_HASH:
            raise RuntimeError("Gemma config hash drift")
        if sha256(MODEL_SNAPSHOT / "model.safetensors.index.json") != MODEL_INDEX_HASH:
            raise RuntimeError("Gemma weight-index hash drift")
        if self.tokenizer_manifest_hash() != TOKENIZER_MANIFEST:
            raise RuntimeError("Gemma tokenizer manifest drift")
        ...
        for arm, expected in (
            ...
            code = (
                ...
                "p='/workspace/project/meritkv_capacity_pressure_20260822/run_capacity_cell.py';"
        ...
        self.run(
            [
                ...
                "/workspace/project/meritkv_capacity_pressure_20260822/tests/"
    ...
    def stop_server(self) -> None:
        if self.run(["docker", "inspect", SERVER_NAME], check=False).returncode == 0:
            ...
            destination.mkdir(parents=True, exist_ok=True)
            (destination / f"{self.retention}_{int(time.time())}.log").write_text(logs)
    ...
    def launch_server(self, label: str) -> None:
        self.stop_server()
        self.retention = "lru"
        self.server_argv = [
            ...
            "--radix-eviction-policy", "lru", "--enable-cache-report", "--enable-metrics",
        ]
        ...
        for _ in range(300):
            ...
            try:
                ...
                ready = (
                    ...
                    and "sglang:swa_evictable_tokens" in metrics
                )
            except Exception:
                pass
            if ready:
                ...
                logs = result.stdout + result.stderr
                if "impl=SWARadixCache" not in logs or "hybrid_swa=True" not in logs:
                    raise RuntimeError(
                        "Gemma startup did not prove SWARadixCache with hybrid SWA enabled"
                    )
                ...
                if not isinstance(canary, dict) or not canary.get("choices"):
                    raise RuntimeError(f"Gemma functional startup canary failed: {canary}")
                canary_dir = CONTROL / "server_canaries"
                canary_dir.mkdir(parents=True, exist_ok=True)
                (canary_dir / f"{label}.json").write_text(
                    json.dumps(canary, indent=2, sort_keys=True) + "\n"
                )
                self.prime_physical_eviction_metrics(label)
                self.log(
                    f"SERVER READY retention=lru label={label} "
                    f"config={self.server_config_hash} cache=SWARadixCache canary=pass"
                )
                return
            running = self.run(
            ...
            if running != "true":
                ...
                logs = result.stdout + result.stderr
                raise RuntimeError(f"Gemma server exited: {logs[-8000:]}")
            time.sleep(5)
        raise RuntimeError("Gemma server did not become ready")
    ...
    def run_cell(
        ...
        command += [
            ...
            "-v", "/datapool/cache/huggingface/hub:/hf_hub:ro",
            "--entrypoint", "python3", IMAGE,
            "/workspace/project/meritkv_capacity_pressure_20260822/run_capacity_cell.py",
            ...
            "--trace_path", f"/workspace/project/meritkv_capacity_pressure_20260822/{trace_relative}",
    ...
    def experiment(self) -> None:
        self.launch_server("default_swa_all_phases")
        self.state("running", "calibration", "separate seed; single predeclared cache cap")
        self.run_cell(
        ...
        smoke_plan = list(
            csv.DictReader((PACKET / "smoke_plan_4.tsv").open(), delimiter="\t")
        )
        if len(smoke_plan) != 4 or {row["arm"] for row in smoke_plan} != set(ARMS_SWA_LRU):
            raise RuntimeError("smoke plan is not the frozen four-arm SWA design")
        for row in smoke_plan:
            if row["retention"] != "lru":
                raise RuntimeError(f"non-LRU row in SWA smoke plan: {row}")
            self.run_cell(
                "smoke", int(row["seed"]), row["arm"], "lru",
                row["trace"], int(row["requests"]),
            )
        self.verify_phase("smoke")
        plan = list(
            csv.DictReader((PACKET / "evaluation_plan_20.tsv").open(), delimiter="\t")
        )
        if len(plan) != 20 or {
            (int(row["seed"]), row["arm"]) for row in plan
        } != {
            (seed, arm) for seed in range(32001, 32006) for arm in ARMS_SWA_LRU
        }:
            raise RuntimeError("evaluation plan is not the frozen five-seed four-arm SWA design")
        for row in plan:
            if row["retention"] != "lru":
                raise RuntimeError(f"non-LRU row in SWA evaluation plan: {row}")
            self.run_cell(
                "evaluation", int(row["seed"]), row["arm"], "lru",
                row["trace"], int(row["requests"]),
            )
        self.stop_server()
    ...
    def restore(self) -> None:
        ...
        actual_config_hash = json_hash(self.normalized_production_config(self.prod_id))
    ...
    def completion_receipt(self) -> None:
        ...
        if (
            ...
            raise RuntimeError(
                "full verifier totals do not match the frozen SWA design: "
    ...
    def execute(self, action: str) -> None:
        ...
        self.state(
            "complete", "verified",
            "25 cells and 2688 requests verified; production restored",
        )
