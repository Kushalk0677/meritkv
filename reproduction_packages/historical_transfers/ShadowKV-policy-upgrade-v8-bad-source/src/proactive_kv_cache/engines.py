...
class ShadowKVPlusEngine(ShadowKVEngine):
    ...
    def __init__(
        ...
        enable_utility_admission: bool = False,
        ...
        self.exact_utility_admission_enabled = bool(enable_utility_admission)
        if min_reuse_prefix_tokens is None:
            if getattr(self.backend, 'backend_name', '') == 'fake':
                ...
                min_reuse_prefix_tokens = int(CONFIG.get('policy.shadowkv_plus.min_reuse_prefix_tokens_cuda', CONFIG.get('policy.lite.min_reuse_prefix_tokens_cuda', 96)))
            else:
                min_reuse_prefix_tokens = int(CONFIG.get('policy.shadowkv_plus.min_reuse_prefix_tokens_cpu', CONFIG.get('policy.lite.min_reuse_prefix_tokens_cpu', 128)))
        self.exact_min_reuse_prefix_tokens = max(int(min_reuse_prefix_tokens), self.bank.min_match_length)
        if utility_min_net_saved_ms is None:
            utility_min_net_saved_ms = float(CONFIG.get('policy.shadowkv_plus.utility_min_net_saved_ms', CONFIG.get('policy.lite.utility_min_net_saved_ms', 0.0)))
        self.exact_utility_min_net_saved_ms = float(utility_min_net_saved_ms)
        self.engine_metrics.update({
            'semantic_ablation_mode': self.semantic_ablation_mode,
            'utility_admission_enabled': self.exact_utility_admission_enabled,
            'exact_min_reuse_prefix_tokens': self.exact_min_reuse_prefix_tokens,
            'exact_utility_min_net_saved_ms': self.exact_utility_min_net_saved_ms,
    ...
    def _slice_prefill_kv_to_prefix(self, kv_cache, prefix_len: int):
        ...
        if prefix_len <= 0 or kv_cache is None:
            return None
        slicer = getattr(self.backend, 'slice_past_key_values', None)
        if callable(slicer):
            sliced = slicer(kv_cache, prefix_len)
            if sliced is not None:
                return sliced
        if isinstance(kv_cache, dict):
            sliced = dict(kv_cache)
            tokens = tuple(kv_cache.get('tokens', ()))[:prefix_len]
            if tokens:
    ...
    def _exact_utility_admission(
        ...
        *,
        ...
        """Utility gate for exact-prefix reuse on the high-hit path.

        This is the Phase 3 patch that matters for latency: a cache candidate
        is not reused merely because it exists.  It is reused only when the
        estimated avoided prefill cost exceeds the measured/learned reuse
        overhead.  This lets ShadowKV++ trade a lower hit rate for better net
        latency and energy efficiency.
        """
        match_len = int(max(match_len, 0))
        self.engine_metrics['reuse_attempts'] = int(self.engine_metrics.get('reuse_attempts', 0)) + 1
        if match_len < int(getattr(self, 'exact_min_reuse_prefix_tokens', self.tuning.min_reuse_prefix_tokens)):
            self.engine_metrics['small_prefix_bypass_total'] = int(self.engine_metrics.get('small_prefix_bypass_total', 0)) + 1
            ...
            self._current_trace_plan = {
                ...
                'policy_reason': f'{mode_label}_small_prefix_bypass',
                'policy_reusable_prefix_tokens': match_len,
                'policy_expected_benefit_ms': 0.0,
                'policy_expected_cost_ms': 0.0,
                'policy_score_ms': 0.0,
                'policy_prefix_length_bucket': 'small_prefix',
                'semantic_ablation_mode': self.semantic_ablation_mode,
            }
            return False

        if not bool(getattr(self, 'exact_utility_admission_enabled', False)):
            return True
        ...
        hint_len = self._shared_prefix_hint(tokens, metadata)
        scaffold_match = hint_len is not None and match_len >= hint_len and self._has_scaffold_hint(tokens, metadata)
        scaffold_discount = float(CONFIG.get('policy.shadowkv_plus.scaffold_overhead_discount', CONFIG.get('policy.lite.scaffold_overhead_discount', 0.45 if scaffold_match else 1.0))) if scaffold_match else 1.0
        min_net = float(CONFIG.get('policy.shadowkv_plus.min_net_saved_ms_scaffold', self.exact_utility_min_net_saved_ms)) if scaffold_match else float(self.exact_utility_min_net_saved_ms)
        ...
        self.engine_metrics['estimated_tokens_saved_total'] = int(self.engine_metrics.get('estimated_tokens_saved_total', 0)) + int(match_len)
        self.engine_metrics['saved_latency_estimate_ms'] = float(self.engine_metrics.get('saved_latency_estimate_ms', 0.0)) + float(max(decision.expected_saved_ms, 0.0))
        self.engine_metrics['net_latency_saved_estimate_ms_total'] = float(self.engine_metrics.get('net_latency_saved_estimate_ms_total', 0.0)) + float(decision.net_utility_ms)
        self._current_trace_plan = {
            'policy_strategy': 'exact',
            'policy_reason': f'{mode_label}_utility_positive_exact',
            'policy_reusable_prefix_tokens': match_len,
            'policy_expected_benefit_ms': float(max(decision.expected_saved_ms, 0.0)),
            'policy_expected_cost_ms': float(decision.expected_cost_ms),
            'policy_score_ms': float(decision.net_utility_ms),
            'policy_prefix_length_bucket': decision.bucket,
            'semantic_ablation_mode': self.semantic_ablation_mode,
    ...
    def serve_tokens(self, request_id: int, tokens: Tuple[int, ...], metadata: Dict | None = None) -> RequestResult:
        ...
        try:
            ...
            if plan.strategy != 'exact' or not self._should_attempt_cache_use(tokens, entry, match_len, metadata=metadata):
                ...
                self.controller.update_feedback(hit=False, wasted_ratio=0.0)
                return self._record(RequestResult(request_id, out.latency_ms + store_latency_ms, 0, len(tokens), False, False, gpu_utilization_pct=out.gpu_utilization_pct))
            if not self._exact_utility_admission(tokens, match_len, metadata=metadata, mode_label='shadowkv_plus'):
                self.bank.record_miss()
                out = self._prefill_full(tokens)
                self.controller.update_feedback(hit=False, wasted_ratio=0.0)
                return self._record(RequestResult(request_id, out.latency_ms, 0, len(tokens), False, False, gpu_utilization_pct=out.gpu_utilization_pct))
...
class ShadowKVPlusLiteEngine(ShadowKVPlusEngine):
    ...
    def __init__(
        ...
        self.exact_min_reuse_prefix_tokens = self.lite_min_reuse_prefix_tokens
        ...
        self.exact_utility_admission_enabled = self.lite_utility_admission_enabled
        if utility_min_net_saved_ms is None:
        ...
        self.exact_utility_min_net_saved_ms = self.lite_utility_min_net_saved_ms
        self.engine_metrics.update({
    ...
    def _should_store_lite_after_full_prefill(self, tokens: Tuple[int, ...], metadata: Dict | None = None) -> bool:
        ...
        if self._is_raw_prompt(metadata):
            return False
        hint = self._shared_prefix_hint(tokens, metadata)
        # Store the same exact scaffold candidates as full ShadowKV++; the
        # minimum reuse threshold belongs to admission, not candidate creation.
        # This prevents Lite from staying at hit_rate=0 when scaffolds are
        # shorter than the current break-even threshold.
        if hint is not None and self._has_scaffold_hint(tokens, metadata):
            return hint >= self.bank.min_match_length
        prefix_len = self._reactive_prefix_len(tokens, metadata)
        min_store = max(int(getattr(self.tuning, 'min_store_prefix_tokens', self.bank.min_match_length)), self.bank.min_match_length)
        if prefix_len < min_store:
            self.engine_metrics['small_prefix_bypass_total'] = int(self.engine_metrics.get('small_prefix_bypass_total', 0)) + 1
            return False
        prefix = tokens[:prefix_len]
        observations = self.bank.get_observation_count(prefix)
