...
def test_shadowkv_plus_lite_bypasses_short_prefixes():
    ...
    # Lite now stores short exact scaffold candidates so candidate discovery is
    # aligned with full ShadowKV++; the admission gate, not the store gate,
    # prevents latency-negative reuse.
    assert engine.engine_metrics['store_successes'] >= 1
    assert engine.engine_metrics['small_prefix_bypass_total'] >= 1
