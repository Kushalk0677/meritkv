...
This means the run is intended to measure executed approximate semantic KV
reuse. Treat it as an experimental measurement, not a production-safety claim.
...
These are the requested Hugging Face-style IDs used by this runner. Public
references consistently mention Gemma 4 26B-A4B, but I did not find a clean
official Hugging Face slug for Gemma 4 12B during setup. If Hugging Face rejects
either ID on the Blackwell machine, change only the two strings in
`GEMMA4_MODELS` inside `blackwell_semantic_n128/run_blackwell_semantic_n128.py`.
...
If Hugging Face access fails, first check whether the account has accepted the
model license/gate and whether the exact model IDs differ from the requested IDs
above. The most likely one to verify is the 12B line.
...
Do not claim a win from semantic matches alone. A useful run should show actual
executed semantic reuse:

```text
semantic_partial_hits > 0
```

and then you should check whether it helped or hurt latency:
...
`comparisons_vs_no_cache.csv`, `all_results.csv`, `_run_manifest.json`,
`_job_results.json`, and the per-cell logs under `_logs/`.
