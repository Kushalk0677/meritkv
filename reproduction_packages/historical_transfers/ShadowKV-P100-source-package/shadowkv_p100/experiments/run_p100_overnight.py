#!/usr/bin/env python3
"""
run_p100_overnight.py -- fast full P100 re-run (single start, complete).

Reruns the ENTIRE P100 matrix with the corrected code:
...
Config:
    n_requests = 64
    seeds      = 42, 123, 456
    engines    = no_cache, shadow_kv, shadow_kv_plus     (3 only)
    models     = 5   x   datasets = 10   x   modes = 3
    -> 5 x 10 x 3 x 3 = 450 isolated runs

Runs everything one-by-one from a single start, isolated subprocess per cell,
resumable (skips completed cells), with a cooldown between each full model.
At the end it writes an aggregate CSV and a zip ready to scp back.
...
COOLDOWN_BETWEEN_MODELS_SEC = 60
RESULTS_DIRNAME = "results_p100_n64_3seed"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fast full P100 re-run (n=64, 3 seeds, 3 engines).")
    ...
    log.info("=" * 80)
    log.info("OVERNIGHT RUN  n=%d  engines=%s  models=%d datasets=%d modes=%d seeds=%d  total=%d",
             args.n_requests, ",".join(ENGINES), len(args.models), len(args.datasets),
             len(args.modes), len(args.seeds), total)
    log.info("  guard=DISABLED  semantic_reuse=EXECUTED(unsafe)  out=%s", root)
    log.info("=" * 80)
