...
NATURAL_CASES = (
    (
        "A support engineer is reviewing a service incident. The deployment remained healthy, "
        "but one availability zone showed elevated request latency for several minutes. The "
        "engineer checks the request trace, cache statistics, and deployment timeline before "
        "writing a concise assessment.\n\nTask: ",
        "Summarize the most likely operational cause and propose two diagnostic checks.",
        "Explain which measurements would distinguish a cache-capacity event from network congestion.",
    ),
    (
        "An analyst is comparing two language-model serving configurations under the same workload. "
        "Both configurations receive identical prompts and use greedy decoding. One configuration "
        "recomputes every prefix, while the other reuses a token-identical cached prefix and computes "
        "only the remaining suffix.\n\nTask: ",
        "Describe the latency components that should be measured in this comparison.",
        "State the correctness checks needed before interpreting any observed speedup.",
    ),
    (
        "A research team collected repeated measurements from several accelerators. Each record "
        "contains mean latency, tail latency, cache occupancy, admitted requests, rejected requests, "
        "and bytes reloaded after eviction. They want a short report that separates direct "
        "measurements from derived ratios.\n\nTask: ",
        "Write a concise description of an appropriate aggregation procedure.",
        "Explain how the team should report variability and derived percentage changes.",
    ),
    (
        "Consider a document-processing system that receives many requests sharing a long instruction "
        "prefix. Some suffixes are short classification questions and others request long summaries. "
        "The cache has limited capacity, so admitting every available prefix can evict entries that "
        "will be reused more often.\n\nTask: ",
        "Explain why cache hit rate alone may not maximize serving utility.",
        "Give an example of a per-request admission rule based on expected benefit and cost.",
    ),
    (
        "A reviewer asks whether two computational paths are equivalent. The first evaluates an entire "
        "token sequence in one forward pass. The second evaluates a shared prefix, retains the model's "
        "native key-value cache, and then evaluates the suffix with explicit attention masks and logical "
        "positions.\n\nTask: ",
        "List the token-level and logit-level comparisons that address the question.",
        "Discuss why floating-point execution may prevent bitwise equality without changing decoded tokens.",
    ),
    (
        "A scheduling policy estimates the time saved by reuse, subtracts lookup and transfer overhead, "
        "and penalizes expected cache pollution. The estimates are calibrated separately for each model "
        "and hardware platform because prefill cost and memory pressure differ substantially across "
        "deployments.\n\nTask: ",
        "Summarize the purpose of hardware-specific calibration.",
        "Explain how the policy should behave when the estimated utility is negative.",
    ),
    (
        "An experiment compares an admission controller with admit-all caching, no caching, a fixed "
        "occupancy limit, and recency-aware and frequency-aware policies. Requests arrive in several "
        "rounds, and a temporary burst creates capacity pressure before the original hot set returns. "
        "The report includes both average and tail latency.\n\nTask: ",
        "Identify the measurements that reveal hot-set recovery after the burst.",
        "Describe how these baselines isolate the value of utility-aware admission.",
    ),
    (
        "A paper evaluates a lightweight decision layer across multiple model families and serving "
        "backends. Some integrations enforce the decision by skipping cache admission, whereas other "
        "integrations are write-through overlays that record decisions while leaving native cache "
        "behavior unchanged.\n\nTask: ",
        "Explain the claim supported by the enforced experiments.",
        "Explain the narrower claim supported by the write-through compatibility experiments.",
    ),
)
...
def make_natural_pair(
    ...
    name = f"natural_{case_index:02d}"
    prefix_text, source_suffix_text, modified_suffix_text = NATURAL_CASES[
        case_index % len(NATURAL_CASES)
    ]
    prefix_ids = tokenizer.encode(prefix_text, add_special_tokens=True)
    source_suffix_ids = tokenizer.encode(source_suffix_text, add_special_tokens=False)
    modified_suffix_ids = tokenizer.encode(modified_suffix_text, add_special_tokens=False)

    minimum_suffix = 8
    prefix_budget = sequence_length - minimum_suffix
    if prefix_budget < 1:
        raise ValueError("Natural-text validation requires room for a nonempty suffix")
    prefix_ids = prefix_ids[:prefix_budget]
    suffix_length = min(
        len(source_suffix_ids),
        len(modified_suffix_ids),
        sequence_length - len(prefix_ids),
    )
    if suffix_length < 1:
        raise ValueError(f"Natural case {name} produced an empty suffix")
    source_ids = prefix_ids + source_suffix_ids[:suffix_length]
    modified_ids = prefix_ids + modified_suffix_ids[:suffix_length]
    if source_ids == modified_ids:
        raise AssertionError(f"Natural case {name} has identical complete sequences")
    ...
    metadata = {
        ...
        "modified_suffix_text": modified_suffix_text,
        "source_token_ids": source_ids,
        "modified_token_ids": modified_ids,
    }
    return source, modified, metadata
...
def main() -> int:
    ...
    if args.input_mode == "natural":
        if args.tiny_model:
            parser.error("Natural-text mode requires --model-id")
    ...
    for sample in range(args.samples):
        ratios = args.shared_ratios if args.input_mode == "random" else (None,)
        for ratio in ratios:
            input_metadata: dict[str, Any]
            if args.input_mode == "natural":
                assert tokenizer is not None
                source_ids, modified_ids, input_metadata = make_natural_pair(
                    tokenizer, sample, args.sequence_length, args.device
                )
                shared_length = next(
                    index
                    for index, (left, right) in enumerate(
                        zip(source_ids[0].tolist(), modified_ids[0].tolist())
                    )
                    if left != right
                )
                actual_ratio = shared_length / modified_ids.shape[1]
            else:
                ...
                actual_ratio = ratio
                input_metadata = {
                    "case_name": f"random_{sample:02d}_{ratio:.2f}",
                    "source_token_ids": source_ids[0].tolist(),
                    "modified_token_ids": modified_ids[0].tolist(),
                }
            print(
                f"sample={sample} mode={args.input_mode} ratio={actual_ratio:.2f} "
                f"shared={shared_length}/{modified_ids.shape[1]}",
                flush=True,
            )
