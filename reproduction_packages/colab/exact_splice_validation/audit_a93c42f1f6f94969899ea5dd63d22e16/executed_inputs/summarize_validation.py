...
def add_numeric(destination: list[float], value: float | None) -> None:
...
def summarize_report(path: Path) -> dict[str, Any]:
    ...
    for case_index, case in enumerate(report["cases"]):
        for layer in case["prefix_cache"]["layers"]:
            add_numeric(cache_max, layer["key"].get("max_abs"))
            add_numeric(cache_max, layer["value"].get("max_abs"))
        add_numeric(suffix_native_full, case["suffix_logits"]["native_vs_full"].get("max_abs"))
        add_numeric(suffix_splice_full, case["suffix_logits"]["splice_vs_full"].get("max_abs"))
        add_numeric(suffix_splice_native, case["suffix_logits"]["splice_vs_native"].get("max_abs"))
        for step in case["generation"]["steps"]:
            add_numeric(generation_native_full, step["native_vs_full"].get("max_abs"))
            add_numeric(generation_splice_full, step["splice_vs_full"].get("max_abs"))
            add_numeric(generation_splice_native, step["splice_vs_native"].get("max_abs"))
...
def main() -> int:
    ...
    summaries = []
    for path in paths:
        if path.name in {args.output_json.name, "combined_summary.json"}:
            continue
        candidate = json.loads(path.read_text(encoding="utf-8"))
        required = {"cases", "configuration", "environment", "summary"}
    ...
    combined = {
        ...
        "all_reports_token_semantic_pass": bool(summaries)
        and all(
            row["summary"].get("all_token_semantic_pass", False)
            for row in summaries
        ),
    }
    ...
    lines = [
        ...
        f"- All token-semantic pass: `{combined['all_reports_token_semantic_pass']}`",
        "",
    ...
    for row in summaries:
        ...
        lines.append(
            ...
            f"{summary['cases']} | {summary.get('token_semantic_passes', 0)} | "
            f"{summary['strict_passes']} | "
