...
def build_engine(args, backend, engine_name: str):
    ...
    if engine_name in SHADOWKV_POLICY_ENGINE_NAMES:
        ...
        raw_strategy = {
            ...
            'shadow_kv_plus_learned': 'strict_utility_gate',
            'shadow_kv_plus_best_latency': 'best_latency',
            'shadow_kv_plus_raw_observer': 'raw_observer',
        }.get(engine_name)
        if engine_name == 'shadow_kv_plus_learned' and not args.learned_policy_path:
            raise ValueError('shadow_kv_plus_learned requires --learned_policy_path')
        return ShadowKVPlusEngine(
            ...
            learned_policy_path=args.learned_policy_path if engine_name == 'shadow_kv_plus_learned' else None,
            learned_policy_threshold=args.learned_policy_threshold,
            outcome_exploration_rate=args.outcome_exploration_rate,
            outcome_exploration_seed=args.outcome_exploration_seed,
        )
...
def _run_shadowkv_policy_tuning(args, requests, output_dir: Path, engine_names: list[str]) -> dict | None:
    ...
    for offset, preset in enumerate(candidates):
        ...
        try:
            ...
            del engine
            if not getattr(args, 'share_backend', False):
                del backend
...
def main() -> None:
    ...
    parser.add_argument('--share_backend', action='store_true', help='Load model once and share across all engines (prevents OOM on small GPUs).')
    ...
    parser.add_argument('--learned_policy_path', default=None, help='JSON learned-admission policy used by shadow_kv_plus_learned.')
    parser.add_argument('--learned_policy_threshold', type=float, default=None, help='Override the admission probability threshold for shadow_kv_plus_learned. Defaults to the threshold saved in the policy JSON, or 0.50 for older JSON files.')
    parser.add_argument('--outcome_exploration_rate', type=float, default=0.0, help='Training-only epsilon rate for forcing bypass-eligible candidates to collect realized outcome labels.')
    parser.add_argument('--outcome_exploration_seed', type=int, default=0, help='Deterministic seed for outcome exploration gating.')
    ...
    try:
        ...
        try:
            if getattr(args, 'share_backend', False) and shared_backend is not None:
                del shared_backend
