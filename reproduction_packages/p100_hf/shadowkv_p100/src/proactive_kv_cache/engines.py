...
class BaseEngine:
    ...
    def _append_policy_trace(self, result: RequestResult) -> None:
        ...
        attempted_admission = str(row.get('policy_strategy') or '') in {'exact', 'semantic_partial'}
        if result.was_cache_hit and result.matched_prefix_length > 0:
            full_mpt = float(row.get('ewma_full_ms_per_token_before') or self.ewma_full_ms_per_token or 0.0)
            if full_mpt <= 0.0:
                full_mpt = float(getattr(self.backend, 'default_ms_per_token', 0.0) or 0.0)
            suffix_estimate_ms = full_mpt * max(int(result.tokens_recomputed), 0)
            full_prefix_estimate_ms = full_mpt * max(int(result.matched_prefix_length), 0)
            observed_reuse_overhead_ms = max(float(result.latency_ms) - suffix_estimate_ms, 0.0)
            outcome_net_saved_ms = full_prefix_estimate_ms - observed_reuse_overhead_ms
            row['outcome_observed_admission'] = True
            row['outcome_net_saved_ms'] = float(outcome_net_saved_ms)
            row['outcome_label_admit_positive'] = 1 if outcome_net_saved_ms > 0.0 else 0
        elif attempted_admission:
            row['outcome_observed_admission'] = True
            row['outcome_net_saved_ms'] = float(-abs(result.latency_ms))
            row['outcome_label_admit_positive'] = 0
        else:
            row['outcome_observed_admission'] = False
...
class ShadowKVEngine(ReactivePrefixCacheEngine):
    ...
    def _speculation_allowed(self, decision_prefix: Tuple[int, ...], decision_benefit_ms: float, decision_cost_ms: float) -> bool:
        ...
        # Memory-breakeven guard (DISABLED by default for the Path C re-run).
        #
        # The original guard computed kv_cost = kappa_mb * 1000 / bw, which
        # over-counts HBM transfer by ~1000x (it omits the MB<->GB /1024
        # conversion), producing a bogus k* ~= 442 for Phi-3 and silently
        # disabling Phi-3 speculation. The physically correct k* is ~10 for
        # every model. We disable the guard so the waste-adaptive speculation
        # machinery (speculation cooldown + health/waste EWMA feedback) must
        # prevent the Phi-3 raw failure on its own. Set the environment
        # variable SHADOWKV_BREAKEVEN_GUARD=1 to restore the original (buggy)
        # guard for a controlled A/B against the committed results.
        import os as _os
        if _os.environ.get('SHADOWKV_BREAKEVEN_GUARD') == '1' and hasattr(self.backend, '_kv_bytes_per_token'):
...
class ShadowKVPlusEngine(ShadowKVEngine):
    ...
    def __init__(
        ...
        self.learned_policy = self._load_learned_policy(learned_policy_path)
        if learned_policy_threshold is None and self.learned_policy is not None:
            threshold_value = self.learned_policy.get('decision_threshold', 0.50)
            self.learned_policy_threshold_source = 'policy_json'
        else:
            threshold_value = 0.50 if learned_policy_threshold is None else learned_policy_threshold
            self.learned_policy_threshold_source = 'cli_or_default'
        self.learned_policy_threshold = max(0.0, min(float(threshold_value), 1.0))
        self.outcome_exploration_rate = max(0.0, min(float(outcome_exploration_rate), 1.0))
        self.outcome_exploration_seed = int(outcome_exploration_seed)
        if self.learned_policy is not None:
            self.name = 'shadow_kv_plus_learned'
        self.engine_metrics.update({
            ...
            'learned_policy_enabled': bool(self.learned_policy is not None),
            'learned_policy_threshold': self.learned_policy_threshold,
            'learned_policy_threshold_source': self.learned_policy_threshold_source,
            'learned_policy_admit_total': 0,
            'learned_policy_bypass_total': 0,
            'learned_policy_flip_to_admit_total': 0,
            'learned_policy_flip_to_bypass_total': 0,
            'outcome_exploration_rate': self.outcome_exploration_rate,
            'outcome_exploration_forced_total': 0,
        })

    def _load_learned_policy(self, path: str | None):
        if not path:
            return None
        with open(path, 'r', encoding='utf-8-sig') as fh:
            payload = json.load(fh)
        required = {'feature_names', 'coef', 'intercept', 'mean', 'scale'}
        missing = required.difference(payload)
        if missing:
            raise ValueError(f'learned policy file {path!r} missing keys: {sorted(missing)}')
        n = len(payload['feature_names'])
        for key in ('coef', 'mean', 'scale'):
            if len(payload[key]) != n:
                raise ValueError(f'learned policy file {path!r} has inconsistent {key} length')
        return payload

    def _learned_feature_value(
        self,
        name: str,
        *,
        tokens: Tuple[int, ...],
        ...
        reusable = int(plan.reusable_prefix_tokens or max(exact_len, sem_prefix_len, 0))
        prefix_ratio = reusable / token_count
        prompt_mode = str(meta.get('prompt_mode', '')).strip().lower()
        values = {
            'matched_prefix_len': float(max(exact_len, sem_prefix_len, 0)),
            'exact_match_len': float(max(exact_len, 0)),
            'semantic_prefix_len': float(max(sem_prefix_len, 0)),
            'semantic_lcp_len': float(max(sem_lcp, 0)),
            'semantic_similarity': float(sem_sim),
            'semantic_match_available': 1.0 if semantic_key is not None else 0.0,
            'token_count': float(token_count),
            'prefix_ratio': float(prefix_ratio),
            'shared_prefix_hint_tokens': float(hint_len or 0),
            'policy_expected_benefit_ms': float(plan.expected_benefit_ms),
            'policy_expected_cost_ms': float(plan.expected_cost_ms),
            'policy_expected_waste_ms': float(plan.expected_waste_ms),
            'policy_confidence': float(plan.confidence),
            'policy_health': float(getattr(getattr(self.controller, 'last_breakdown', None), 'health', 0.0) or 0.0),
            'ewma_hit_rate': float(getattr(self.controller.policy, '_ewma_hit_rate', 0.0)),
            'ewma_waste_ratio': float(getattr(self.controller.policy, '_ewma_waste_ratio', 0.0)),
            'is_templated': 1.0 if prompt_mode == 'templated' else 0.0,
            'is_semantic': 1.0 if prompt_mode == 'semantic' else 0.0,
            'is_rag': 1.0 if prompt_mode == 'rag' else 0.0,
            'is_raw': 1.0 if prompt_mode == 'raw' else 0.0,
        }
        return float(values.get(name, 0.0))

    def _learned_admit_probability(
        self,
        *,
        tokens: Tuple[int, ...],
        exact_len: int,
        ...
        if self.learned_policy is None:
            return None
        names = list(self.learned_policy['feature_names'])
        coef = [float(x) for x in self.learned_policy['coef']]
        mean = [float(x) for x in self.learned_policy['mean']]
        scale = [float(x) if abs(float(x)) > 1e-12 else 1.0 for x in self.learned_policy['scale']]
        z = float(self.learned_policy.get('intercept', 0.0))
        for i, name in enumerate(names):
            raw = self._learned_feature_value(
                name,
                tokens=tokens,
                exact_len=exact_len,
                sem_sim=sem_sim,
                sem_prefix_len=sem_prefix_len,
                sem_lcp=sem_lcp,
    ...
    def _apply_learned_policy(
        ...
        *,
        ...
        p_admit = self._learned_admit_probability(
            tokens=tokens,
            exact_len=exact_len,
            sem_sim=sem_sim,
            sem_prefix_len=sem_prefix_len,
        ...
        if p_admit is None:
            return plan, semantic_key, None

        learned_admit = p_admit >= self.learned_policy_threshold
        self.engine_metrics['learned_policy_admit_total' if learned_admit else 'learned_policy_bypass_total'] = int(
            self.engine_metrics.get('learned_policy_admit_total' if learned_admit else 'learned_policy_bypass_total', 0)
        ) + 1

        original_strategy = plan.strategy
        reason = f'learned_p={p_admit:.3f};original={original_strategy};{plan.reason}'
        if not learned_admit:
            if original_strategy != 'bypass':
                self.engine_metrics['learned_policy_flip_to_bypass_total'] = int(self.engine_metrics.get('learned_policy_flip_to_bypass_total', 0)) + 1
            return ReusePlan(
                strategy='bypass',
                speculate_depth_tokens=0,
                ...
                score=float(p_admit - self.learned_policy_threshold),
                confidence=float(max(p_admit, 1.0 - p_admit)),
                reason='learned_bypass:' + reason,
            ), None, p_admit

        if original_strategy != 'bypass':
            return ReusePlan(
                strategy=original_strategy,
                speculate_depth_tokens=int(plan.speculate_depth_tokens),
                reusable_prefix_tokens=int(plan.reusable_prefix_tokens),
                ...
                expected_waste_ms=float(plan.expected_waste_ms),
                score=float(p_admit - self.learned_policy_threshold),
                confidence=float(max(p_admit, plan.confidence)),
                reason='learned_admit:' + reason,
            ), semantic_key, p_admit

        if exact_len > 0:
            self.engine_metrics['learned_policy_flip_to_admit_total'] = int(self.engine_metrics.get('learned_policy_flip_to_admit_total', 0)) + 1
            return ReusePlan(
                strategy='exact',
                speculate_depth_tokens=int(max(plan.reusable_prefix_tokens, exact_len)),
                reusable_prefix_tokens=int(max(plan.reusable_prefix_tokens, exact_len)),
                layer_reuse_ratio=float(plan.layer_reuse_ratio or 1.0),
                expected_benefit_ms=float(plan.expected_benefit_ms),
                expected_cost_ms=float(plan.expected_cost_ms),
                expected_waste_ms=float(plan.expected_waste_ms),
                score=float(p_admit - self.learned_policy_threshold),
                confidence=float(max(p_admit, plan.confidence)),
                reason='learned_exact:' + reason,
            ), semantic_key, p_admit

        if semantic_key is not None and sem_prefix_len > 0 and (self.allow_approximate_semantic_reuse or self.semantic_ablation_mode in self._semantic_execution_modes):
            self.engine_metrics['learned_policy_flip_to_admit_total'] = int(self.engine_metrics.get('learned_policy_flip_to_admit_total', 0)) + 1
            reusable = int(max(plan.reusable_prefix_tokens, min(sem_prefix_len, len(tokens))))
            return ReusePlan(
                strategy='semantic_partial',
                speculate_depth_tokens=reusable,
                reusable_prefix_tokens=reusable,
                layer_reuse_ratio=float(plan.layer_reuse_ratio or min(self.controller.max_layer_reuse_ratio, 0.35)),
                expected_benefit_ms=float(plan.expected_benefit_ms),
                expected_cost_ms=float(plan.expected_cost_ms),
                expected_waste_ms=float(plan.expected_waste_ms),
                score=float(p_admit - self.learned_policy_threshold),
                confidence=float(max(p_admit, plan.confidence)),
                reason='learned_semantic:' + reason,
            ), semantic_key, p_admit
        ...
        return ReusePlan(
            ...
            score=float(p_admit - self.learned_policy_threshold),
            confidence=float(max(p_admit, 1.0 - p_admit)),
            reason='learned_no_executable_candidate:' + reason,
        ), None, p_admit

    def _should_explore_outcome(self, tokens: Tuple[int, ...], metadata: Dict | None = None) -> bool:
        if self.outcome_exploration_rate <= 0.0:
            return False
        # Deterministic pseudo-random gate: stable across reruns and independent
        # of Python's process-randomized hash(). Include a request counter so
        # repeated templated prompts do not collapse to a single exploration draw.
        h = 2166136261 ^ int(self.outcome_exploration_seed)
        request_index = (metadata or {}).get('request_index', self.engine_metrics.get('requests_seen', 0))
        try:
            h ^= int(request_index) & 0xFFFFFFFF
            h = (h * 16777619) & 0xFFFFFFFF
        except (TypeError, ValueError):
            for ch in str(request_index):
                h ^= ord(ch) & 0xFFFFFFFF
                h = (h * 16777619) & 0xFFFFFFFF
        for tok in tokens[:64]:
            h ^= int(tok) & 0xFFFFFFFF
            h = (h * 16777619) & 0xFFFFFFFF
        bucket = (h % 10000) / 10000.0
        return bucket < self.outcome_exploration_rate

    def _apply_outcome_exploration(
        self,
        plan: ReusePlan,
        *,
        tokens: Tuple[int, ...],
        ...
        if plan.strategy != 'bypass' or not self._should_explore_outcome(tokens, metadata):
            return plan, semantic_key, False
        if exact_len > 0:
            self.engine_metrics['outcome_exploration_forced_total'] = int(self.engine_metrics.get('outcome_exploration_forced_total', 0)) + 1
            return ReusePlan(
                strategy='exact',
                speculate_depth_tokens=max(int(plan.reusable_prefix_tokens), int(exact_len)),
                reusable_prefix_tokens=max(int(plan.reusable_prefix_tokens), int(exact_len)),
                layer_reuse_ratio=float(plan.layer_reuse_ratio or 1.0),
                expected_benefit_ms=float(plan.expected_benefit_ms),
                expected_cost_ms=float(plan.expected_cost_ms),
                expected_waste_ms=float(plan.expected_waste_ms),
                ...
                reason='outcome_explore_exact:' + str(plan.reason),
            ), semantic_key, True
        if semantic_key is not None and sem_prefix_len > 0 and (self.allow_approximate_semantic_reuse or self.semantic_ablation_mode in self._semantic_execution_modes):
            self.engine_metrics['outcome_exploration_forced_total'] = int(self.engine_metrics.get('outcome_exploration_forced_total', 0)) + 1
            reusable = int(max(plan.reusable_prefix_tokens, min(sem_prefix_len, len(tokens))))
            return ReusePlan(
                ...
                score=float(plan.score),
                confidence=float(plan.confidence),
                reason='outcome_explore_semantic:' + str(plan.reason),
            ), semantic_key, True
        return plan, semantic_key, False
    ...
    def _plan_for_request(self, tokens: Tuple[int, ...], match, metadata: Dict | None = None):
        ...
        plan, semantic_key, learned_p = self._apply_learned_policy(
            plan,
            tokens=tokens,
            exact_len=exact_len,
            sem_sim=sem_sim,
        ...
        plan, semantic_key, explored = self._apply_outcome_exploration(
            plan,
            tokens=tokens,
            exact_len=exact_len,
            semantic_key=semantic_key,
        ...
        self._current_trace_plan = {
            'policy_strategy': plan.strategy,
            'semantic_ablation_mode': self.semantic_ablation_mode,
            'outcome_exploration_forced': bool(explored),
            'policy_speculate_depth_tokens': int(plan.speculate_depth_tokens),
            'policy_reusable_prefix_tokens': int(plan.reusable_prefix_tokens),
            'policy_layer_reuse_ratio': float(plan.layer_reuse_ratio),
            'policy_expected_benefit_ms': float(plan.expected_benefit_ms),
        ...
        if learned_p is not None:
            self._current_trace_plan['learned_policy_probability'] = float(learned_p)
            self._current_trace_plan['learned_policy_threshold'] = float(self.learned_policy_threshold)
